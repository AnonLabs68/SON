...
const milestones = [
  'Institutional-grade execution layer with LAN/WAN peer discovery and real-time health scoring.',
  'Kinetic hot/cold state architecture supporting robust replayable observed market ranges.',
  'Enterprise peer backfill ensuring new nodes hydrate instantly from active network terminals.',
  'Hardware-secured node passport integrating verified Solana identity receipts.',
  'Advanced Oracle/Cortex signal pipeline producing highly inspectable derivative outputs.',
  'Developer SDK package featuring comprehensive technical documentation and live telemetry tools.',
];
      ...
      <main id="top" className="relative pt-16">
        <section className="relative min-h-[92vh] overflow-hidden px-5 py-24 md:px-8 md:py-32">
          <div className="absolute inset-0 z-0">
            <div className="absolute top-1/4 left-1/4 h-96 w-96 rounded-full bg-[#00F5FF]/10 blur-[100px] animate-orb" />
            <div className="absolute bottom-1/4 right-1/4 h-80 w-80 rounded-full bg-[#22c55e]/10 blur-[100px] animate-orb" style={{ animationDelay: '2s' }} />
            ...
            <div className="absolute inset-0 bg-[linear-gradient(to_right,rgba(255,255,255,0.035)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.035)_1px,transparent_1px)] bg-[size:56px_56px] opacity-30 animate-grid" />
          </div>
          <div className="relative z-10 mx-auto grid w-full max-w-7xl min-w-0 items-center gap-12 lg:grid-cols-[1.02fr_0.98fr]">
            <FadeIn className="min-w-0 max-w-[calc(100vw-40px)] sm:max-w-full">
              <div className="mb-6 inline-flex items-center gap-2 border border-theme-accent/40 bg-theme-accent-dim/30 px-3 py-2 font-mono text-[11px] uppercase tracking-[0.18em] text-theme-accent glass-panel">
                <BadgeCheck className="h-3.5 w-3.5 text-theme-accent" />
                Sovereign Core // Developer Preview
              </div>
              ...
              <p className="mt-7 max-w-[320px] break-words text-base leading-7 text-theme-text-secondary sm:max-w-2xl sm:text-lg sm:leading-8 md:text-xl">
                A native C++ Solana execution environment merging an institutional dark pool terminal, Oracle/Cortex intelligence, and a Quantum agentic layer. Built for uncompromising HFT performance.
              </p>
              <div className="mt-8 flex max-w-[320px] flex-col gap-3 sm:max-w-none sm:flex-row">
                <a href={DEMO_URL} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 bg-theme-accent px-5 py-3 font-semibold text-black transition-all hover:scale-105 hover:bg-[#44fbff] hover:shadow-[0_0_20px_rgba(0,245,255,0.4)]">
                  Watch live demo <ArrowRight className="h-4 w-4" />
                </a>
                <a href={PUBLIC_REPO_URL} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 border border-theme-border bg-white/[0.03] px-5 py-3 font-semibold text-white transition-all hover:scale-105 hover:border-theme-accent/50 hover:bg-white/[0.06] glass-panel">
                  Access Developer SDK <Github className="h-4 w-4" />
                </a>
              </div>
              <div className="mt-9 grid max-w-[320px] gap-3 sm:max-w-none sm:grid-cols-2">
                {proofPoints.map((point) => (
                  <div key={point.label} className="border border-theme-border bg-black/20 p-4 glass-panel magnetic-card">
            ...
            {productLayers.map((layer, index) => {
              ...
              return (
                <FadeIn key={layer.title} delay={index * 0.05}>
                  <div className="h-full border border-theme-border bg-[#090a0d] p-6 transition-colors magnetic-card glass-panel">
            ...
            {[
              ...
              return (
                <FadeIn key={item.title} delay={index * 0.08}>
                  <div className="border border-theme-border bg-[#08090c] p-7 magnetic-card glass-panel">
        ...
        <Section id="milestones">
          <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr]">
            <FadeIn>
              <div className="font-mono text-[12px] uppercase tracking-[0.2em] text-theme-accent">Engineering Pipeline</div>
              <h2 className="mt-4 text-3xl font-bold tracking-normal text-white md:text-5xl">
                Active institutional-grade development.
              </h2>
              <p className="mt-5 text-lg leading-8 text-theme-text-secondary">
                Sovereign is engineered as the premier execution environment for the Solana ecosystem, currently rolling out measured architectural milestones to our developer network.
              </p>
            </FadeIn>
            <div className="grid gap-4">
              {milestones.map((milestone, index) => (
                <FadeIn key={milestone} delay={index * 0.05}>
                  <div className="flex gap-4 border border-theme-border bg-white/[0.025] p-4 glass-panel magnetic-card">
        ...
        <Section className="border-t border-theme-border bg-[#07090d] text-center relative overflow-hidden">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(0,245,255,0.1),transparent_40%)]" />
          <FadeIn className="relative z-10">
            <Globe2 className="mx-auto mb-6 h-9 w-9 text-theme-accent animate-pulse-slow" />
            <h2 className="mx-auto max-w-4xl text-3xl font-bold tracking-normal text-white md:text-5xl">
              Engineered for the bleeding edge of market execution.
            </h2>
            <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-theme-text-secondary">
              Sovereign is in active alpha development. The current release offers uncompromised native workstation capabilities, robust SON telemetry, and Solana-verifiable identity events.
            </p>
            <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
              <a href={PUBLIC_REPO_URL} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 bg-theme-accent px-5 py-3 font-semibold text-black transition-all hover:scale-105 hover:bg-[#44fbff] hover:shadow-[0_0_20px_rgba(0,245,255,0.4)]">
                Access Developer SDK <ArrowRight className="h-4 w-4" />
              </a>
              <a href={EMAIL} className="inline-flex items-center justify-center gap-2 border border-theme-border bg-white/[0.03] px-5 py-3 font-semibold text-white transition-all hover:scale-105 hover:border-theme-accent/50 hover:bg-white/[0.06] glass-panel">
                Contact Enterprise
              </a>
            </div>
          </FadeIn>
